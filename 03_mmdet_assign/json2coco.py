...
coco_dict['info']['contributor'] = '123'
...
for image_id, image_info in annotations.items():
    ...
    # 获取图像的height和width
    img = cv2.imread(os.path.join('../data/balloon/val/', filename))
    ...
    for region_id, region_info in regions.items():
        ...
        # 添加注释信息
        annotation = {'id': annotation_id, 'image_id': int(image_id), 'category_id': category_id, 'segmentation': [points],
                      'area': width * height, 'bbox': [xmin, ymin, width, height], 'iscrowd': 0}
        coco_dict['annotations'].append(annotation)
        annotation_id += 1

    # 保存COCO格式文件
with open('../data/balloon/val/balloon_val.json', 'w') as f:
